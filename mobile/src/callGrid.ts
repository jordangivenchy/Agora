/* The gallery's arrangement, the way Discord lays out a call: every camera
   ...
   screen on its own runs the full width at 16:9. */
...
export function planGrid(kinds: GridKind[], width: number, height: number, gap: number): GridPlan {
  ...
  for (let cols = minCols; cols <= Math.min(MAX_COLS, Math.max(minCols, kinds.length * 2)); cols++) {
    ...
    /* Height in windows: a one-column screen row is 16:9 of the width. */
    const units = rows.reduce((sum, row) => sum + (cols === 1 && kinds[row[0]] === "screen" ? 9 / 16 : 1), 0);
  ...
  const rowH = (row: number[]) => (cols === 1 && kinds[row[0]] === "screen" ? Math.round(size * 9 / 16) : size);
