...
/* A link in the post is the one piece of markup a reader can use, so it
   survives: everything else (headings, bullets, emphasis, code, images)
   is stripped, and `[label](url)` and bare URLs are carried through for
   the card to render. Stripping runs between the links, never over them
   — an emphasis pass across a URL would eat its underscores. */
