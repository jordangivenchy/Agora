...
export default function ModPage({ initial }: {
  /** The first view, fetched by the route on the server (already gated):
      the page renders with it at once and re-checks in the browser. */
  initial?: Extract<ModInitial, { status: "ok" }>;
}) {
  ...
  const [loadError, setLoadError] = useState<string | null>(initial?.loadError ?? null);
