...
/* Discord's Interactions Endpoint: the "Get my beta key" button on the
   ...
   ephemeral message: only the person who asked sees their key, a
   one-time key minted for their Discord account (lib/betaKeys). The
   master code never shows here. /api/webhook/* is beta-gate exempt
   (src/proxy.ts). */
...
export async function POST(request: NextRequest) {
  ...
  let state: BetaKeyState;
  if (!code) {
    state = { kind: "open" };
  } else if (!hasAdminCredentials() || !who?.id) {
    state = { kind: "unavailable" };
  } else {
    try {
      state = await mintBetaKey({ id: who.id, name: who.username ?? null });
    } catch (e) {
      console.error("[discord] key desk failed:", e);
      state = { kind: "unavailable" };
    }
  }
  console.log(`[discord] key desk: ${state.kind} for ${who?.username ?? "?"} (${who?.id ?? "?"})`);
  return reply({ embeds: [betaKeyEmbed(state, origin)] });
}
