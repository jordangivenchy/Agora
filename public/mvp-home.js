...
// everything else. Keep the window pinned instead: the scroll boxes end
// at the keyboard (html.kb-open + --kb; mvp-home.css and globals.css)
// and the field is scrolled clear of it inside its own box, while the
// tab bar stays put behind the keyboard like a native app's. Inactive
// where the keyboard shrinks the layout itself (Android) or on desktop.
(function () {
  const vv = window.visualViewport;
  if (!vv || !document.querySelector('main.main')) return;
  const phone = window.matchMedia('(max-width: 639px)');
  const root = document.documentElement;
  let open = false, raf = 0;
  const editable = (el) => !!el && el !== document.body &&
  ...
  const sync = () => {
    ...
    if (now !== open) {
      ...
      // The touch lock below is only on the document while the keyboard is up.
  ...
  window.addEventListener('scroll', () => { if (open && window.scrollY) window.scrollTo(0, 0); }, { passive: true });
  document.addEventListener('focusin', queue);
  document.addEventListener('focusout', () => setTimeout(queue, 60));
  ...
  let ty = 0;
  document.addEventListener('touchstart', (e) => { ty = e.touches[0].clientY; }, { passive: true, capture: true });
  function lock(e) {
    if (!open || e.touches.length !== 1) return;
    if (!absorbs(e.target, e.touches[0].clientY - ty)) e.preventDefault();
  }
  window.__agoraKeyboardGuard = sync;
})();
...
// The navbar (messages button, avatar menu) is React now: components/SiteNavbar.tsx.
...
/* ═══════════════════════════════════════════
   EXPLORE PAGE — React now (components/ExplorePage.tsx)
═══════════════════════════════════════════ */
...
// ═══════════════════════════════════════════════
//  STARFIELD — React now (components/Starfield.tsx)
// ═══════════════════════════════════════════════
