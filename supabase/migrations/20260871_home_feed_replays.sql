-- Add debate recordings (replays) to the home feed.
-- Recreated from the live get_home_feed definition with three additions:
--   1. the `rooms` CTE now also pulls ENDED rooms that have a recording,
--      paginated by ended_at (live/scheduled stay page-1-only);
--   2. a `replay_items` CTE emits kind='replay' for them, scored by
--      affinity + recency like posts;
--   3. replay_items joins the final union.
