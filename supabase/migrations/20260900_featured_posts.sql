-- Featured posts: a site moderator puts a post on the home page's
-- carousel ("Feature on home" in the post's menu). featured_at says
-- when; null means not featured; the home page shows the newest three
-- from the last fortnight. Only site moderators may set it, by any
-- path (the guard trigger), and set_post_featured is what the app
-- calls — it runs as its definer so a moderator can feature a post
-- they could not otherwise update.
...
create index if not exists community_posts_featured_idx
  on public.community_posts (featured_at desc) where featured_at is not null;

create or replace function public.guard_post_featured() returns trigger
language plpgsql set search_path = public as $$
begin
  if (tg_op = 'INSERT' and new.featured_at is not null)
     or (tg_op = 'UPDATE' and new.featured_at is distinct from old.featured_at) then
    if not exists (select 1 from public.users where id = auth.uid() and is_moderator) then
      raise exception 'only site moderators can feature posts' using errcode = '42501';
    end if;
  end if;
  return new;
end $$;
drop trigger if exists trg_guard_post_featured on public.community_posts;
create trigger trg_guard_post_featured
  before insert or update on public.community_posts
  for each row execute function public.guard_post_featured();

create or replace function public.set_post_featured(p_post uuid, p_featured boolean) returns timestamptz
...
begin
  ...
  update public.community_posts
    ...
    where id = p_post
    returning featured_at into v;
  if not found then raise exception 'no such post'; end if;
  return v;
end $$;
revoke all on function public.set_post_featured(uuid, boolean) from public, anon;
grant execute on function public.set_post_featured(uuid, boolean) to authenticated;

-- The boards' post list carries featured_at, so the menu can say
-- "Feature on home" or "Remove from home". The return type changes,
